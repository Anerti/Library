...
- **DB schema:** Hibernate `ddl-auto=validate` — schema is managed externally via `src/main/resources/db/schema.sql` (Flyway-like through POJA pipeline), not by JPA auto-DDL. Uses native PostgreSQL types (`user_role`, `sale_status`).
- **OpenAPI:** Hand-authored specs at `doc/openapi.yml` + `doc/api.yml` (3.0.3, 27 paths, 58 ops). API base `/api/v1`.
- **Auth — JWT bearer token:** All POST/PATCH/DELETE endpoints require `ADMIN` role except `/api/v1/auth/register` and `/api/v1/auth/login`. `GET` endpoints are public. JWT is extracted by `JwtAuthenticationFilter` (extends `OncePerRequestFilter`). Subject is the user UUID — no `UserDetailsService` / DB hit per request.
- **Password encoding:** Argon2 via `Spring Security Crypto`'s `Argon2PasswordEncoder`.
- **User entity:** The `User` entity replaces the former `Customer` entity. `Customer` still exists as a DB-level concept through the user's `Role.CUSTOMER` — the `CustomerService` now delegates to `UserService`.
...
- **Validation:** Centralised in `DataValidator` (package `validator`) for shared rules (email, phone, ISBN, safe strings). Domain-specific validators (`AuthorValidator`, `BookCopyValidator`, `UserValidator`, etc.) handle per-domain rules. No JSR-380 annotations on DTOs.
- **Exceptions:** Standardised hierarchy — `BadRequestException` (400), `UnauthorizedException` (401), `ForbiddenException` (403), `NotFoundException` (404), `ConflictException` (409), `UnprocessableEntityException` (422). All handled by `GlobalExceptionHandler` which returns JSON `ErrorBody`.
...
- **Controller tests:** `@WebMvcTest` with `@MockBean` services and `MockMvc`. `GlobalExceptionHandler` is wired as an extra controller. Add `@MockBean JwtTokenProvider` and `@AutoConfigureMockMvc(addFilters=false)` to bypass auth in controller tests.
...
- **Enum entities:** `BookCopyFormat` (HARDCOVER, PAPERBACK, POCKET), `BookCopyStatus` (AVAILABLE, SOLD_OUT), `SaleStatus` (SOLD, BOOKED, EXPIRED), `Role` (ADMIN, CUSTOMER).
...
- On native PG types (`user_role`, `sale_status`), cast in native queries: `::user_role` or `::sale_status`. Don't switch to `@JdbcTypeCode`.
- Controller tests using `@WebMvcTest` need `@MockBean JwtTokenProvider` and `@AutoConfigureMockMvc(addFilters=false)` to avoid JWT auth interferring with endpoint tests.
