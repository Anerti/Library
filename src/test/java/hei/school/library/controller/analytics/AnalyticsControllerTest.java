...
class AnalyticsControllerTest {
  ...
  void should_return_revenue_by_genre_with_default_params() throws Exception {
    ...
    when(analyticsService.findRevenueByGenre(
            eq(libraryId), any(), any(), eq("desc"), eq(1), eq(20)))
        .thenReturn(response);
    ...
    verify(analyticsService).findRevenueByGenre(libraryId, null, null, "desc", 1, 20);
  }
  ...
  void should_pass_query_params_to_service() throws Exception {
    ...
    when(analyticsService.findRevenueByGenre(eq(libraryId), any(), any(), eq("asc"), eq(2), eq(10)))
        .thenReturn(response);
    ...
    verify(analyticsService)
        .findRevenueByGenre(
            libraryId, LocalDate.of(2026, 1, 1), LocalDate.of(2026, 1, 31), "asc", 2, 10);
  }
