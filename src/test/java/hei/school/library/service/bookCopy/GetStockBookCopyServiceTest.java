...
public class GetStockBookCopyServiceTest {
  ...
  void getStock_shouldReturnDifference_receivedMinusSold() {
    ...
    when(stockRepository.calculateStock(bookId, "PAPERBACK", null)).thenReturn(List.of(38L, 25L));
  ...
  void getStock_shouldReturnZero_whenAllSold() {
    when(bookRepository.existsById(bookId)).thenReturn(true);
    when(stockRepository.calculateStock(bookId, null, null)).thenReturn(List.of(0L));
  ...
  void getStock_shouldReturnZero_whenNoCopiesReceived() {
    when(bookRepository.existsById(bookId)).thenReturn(true);
    when(stockRepository.calculateStock(bookId, null, null)).thenReturn(List.of());
