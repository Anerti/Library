...
curlie POST "http://localhost:8080/libraries" name="" phone="+261 33 44 55 77" email="no-name@test.mg" address="Main Street"
echo

echo "── 5) 422 — POST /libraries (name with digits)  →  422 / forbidden characters"
curlie POST "http://localhost:8080/libraries" name="Library1" phone="+261 33 44 55 77" email="bad-name@test.mg" address="Main Street"
echo

echo "── 6) 422 — POST /libraries (phone blank)  →  422 / phone is required and cannot be blank"
curlie POST "http://localhost:8080/libraries" name="Central Library" phone="" email="no-phone@test.mg" address="Main Street"
echo

echo "── 7) 422 — POST /libraries (phone invalid)  →  422 / Invalid phone format"
curlie POST "http://localhost:8080/libraries" name="Central Library" phone="abc" email="bad-phone@test.mg" address="Main Street"
echo

echo "── 8) 422 — POST /libraries (email blank)  →  422 / email is required and cannot be blank"
curlie POST "http://localhost:8080/libraries" name="Central Library" phone="+261 33 44 55 77" email="" address="Main Street"
echo

echo "── 9) 422 — POST /libraries (email invalid)  →  422 / Invalid email format"
curlie POST "http://localhost:8080/libraries" name="Central Library" phone="+261 33 44 55 77" email="not-an-email" address="Main Street"
echo

echo "── 10) 422 — POST /libraries (address blank)  →  422 / address is required and cannot be blank"
curlie POST "http://localhost:8080/libraries" name="Central Library" phone="+261 33 44 55 77" email="no-address@test.mg" address=""
echo

echo "── 11) 422 — POST /libraries (address invalid chars)  →  422 / invalid characters"
curlie POST "http://localhost:8080/libraries" name="Central Library" phone="+261 33 44 55 77" email="bad-address@test.mg" address="123 Main St|"
echo
