...
curlie -H "Authorization:Bearer $ADMIN_TOKEN" PATCH "http://localhost:8080/genres/${CONFLICT_ID}" name="PatchConflictTarget"
echo

echo "── 3) 200 — PATCH /genres/{id} (unique name)  →  200 / UpdatedSuccessName"
curlie -H "Authorization:Bearer $ADMIN_TOKEN" PATCH "http://localhost:8080/genres/${SUCCESS_ID}" name="UpdatedSuccessName"
echo

echo "── 4) 404 — PATCH /genres/{id} (non-existent UUID)  →  404 / not found"
curlie -H "Authorization:Bearer $ADMIN_TOKEN" PATCH "http://localhost:8080/genres/00000000-0000-0000-0000-000000000000" name="Ghost"
echo

echo "── 5) 400 — PATCH /genres/{id} (invalid UUID)  →  400 / bad request"
curlie -H "Authorization:Bearer $ADMIN_TOKEN" PATCH "http://localhost:8080/genres/not-a-uuid" name="Irrelevant"
echo

echo "── 6) 422 — PATCH /genres/{id} (blank name)  →  422 / name is required"
curlie -H "Authorization:Bearer $ADMIN_TOKEN" PATCH "http://localhost:8080/genres/${SUCCESS_ID}" name=""
echo

echo "── 7) 422 — PATCH /genres/{id} (name with digits)  →  422 / forbidden characters"
curlie -H "Authorization:Bearer $ADMIN_TOKEN" PATCH "http://localhost:8080/genres/${SUCCESS_ID}" name="Genre1"
echo

echo "── 8) 422 — PATCH /genres/{id} (name with underscore)  →  422 / forbidden characters"
curlie -H "Authorization:Bearer $ADMIN_TOKEN" PATCH "http://localhost:8080/genres/${SUCCESS_ID}" name="Fan_tasy"
echo
